...
from school_program_intelligence.shared.models import Study
from school_program_intelligence.b_spatial.graph import build_circulation_graph, space_to_node
...
def spatial_fingerprints(study: Study) -> dict[str, dict[str, float]]:
    ...
    entrances = study.floor_plan.entrances
    fingerprints: dict[str, dict[str, float]] = {}

    undirected = graph
    for space in study.floor_plan.spaces:
...
def spatial_fit(study: Study, assignments: dict[str, str]) -> dict[str, float]:
