...
def _space_for_program(assignment: Assignment, program_id: str) -> str | None:
    return assignment.mapping.get(program_id)


def check_program_presence(study: Study, assignment: Assignment) -> list[CheckResult]:
    results: list[CheckResult] = []
    assigned_spaces = set(assignment.mapping.values())
    for program in study.programs:
        if not program.required:
            continue
        space_id = _space_for_program(assignment, program.id)
        if not space_id:
            results.append(
                CheckResult(
        ...
        elif space_id not in study.space_map():
            results.append(
                CheckResult(
                    metric_id="compliance.presence",
                    status="fail",
    ...
    for pid, sid in assignment.mapping.items():
...
def check_area_fit(study: Study, assignment: Assignment) -> list[CheckResult]:
    results: list[CheckResult] = []
    spaces = study.space_map()
    for program in study.programs:
        space_id = _space_for_program(assignment, program.id)
...
def check_capacity_fit(study: Study, assignment: Assignment) -> list[CheckResult]:
    results: list[CheckResult] = []
    spaces = study.space_map()
...
def check_hard_relationships(study: Study, assignment: Assignment, distances: dict[tuple[str, str], float] | None = None) -> list[CheckResult]:
    ...
    for rel in study.relationships:
        if not rel.hard:
            continue
        s_a = _space_for_program(assignment, rel.source_program)
        s_b = _space_for_program(assignment, rel.target_program)
...
def evaluate_compliance(study: Study, assignment: Assignment, distances: dict[tuple[str, str], float] | None = None) -> SimulationReport:
    checks: list[CheckResult] = []
    checks.extend(check_program_presence(study, assignment))
    checks.extend(check_area_fit(study, assignment))
    checks.extend(check_capacity_fit(study, assignment))
    checks.extend(check_hard_relationships(study, assignment, distances=distances))
