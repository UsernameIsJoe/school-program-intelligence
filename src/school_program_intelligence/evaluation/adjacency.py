...
from school_program_intelligence.program.schema import Importance, Project, Scheme
...
def adjacency_metrics(project: Project, scheme: Scheme) -> dict:
    pairs = []
    total_penalty = 0.0
    weight_sum = 0.0
    for rel in project.relationships:
        dist = program_pair_distance(
            project, scheme.assignments, rel.source_program, rel.target_program
        )
